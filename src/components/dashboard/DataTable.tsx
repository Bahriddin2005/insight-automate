...
export default function DataTable({ data, columns }: DataTableProps) {
  ...
  const exportExcel = () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'cleaned_data.xlsx'; a.click();
    URL.revokeObjectURL(url);
  };
    ...
    >
      <div className="p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3 border-b border-border/50">
        <div className="relative flex-1 w-full sm:max-w-xs">
          ...
          <input
            ...
            className="w-full bg-secondary text-secondary-foreground pl-9 pr-3 py-2 rounded-lg text-sm border border-border focus:ring-1 focus:ring-primary outline-none"
          />
        </div>
          ...
          <tbody>
            {pageData.map((row, i) => (
              <tr key={i} className="border-b border-border/20 hover:bg-secondary/20 transition-colors">
                {columns.filter(c => c.type !== 'id').slice(0, 12).map(col => (
                  <td key={col.name} className="px-4 py-2.5 whitespace-nowrap text-foreground/80 data-font text-xs">
      ...
      <div className="p-4 flex items-center justify-between border-t border-border/50">
        <span className="text-xs text-muted-foreground">
          {sorted.length.toLocaleString()} rows · Page {page + 1} of {totalPages || 1}
        </span>
        <div className="flex gap-1">
          <Button variant="ghost" size="icon" disabled={page === 0} onClick={() => setPage(p => p - 1)} className="h-8 w-8">
            <ChevronLeft className="w-4 h-4" />
          </Button>
          <Button variant="ghost" size="icon" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="h-8 w-8">
